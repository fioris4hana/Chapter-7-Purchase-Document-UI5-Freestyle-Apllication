	...
	return {
		...
		_initAnnotationMockServer: function (sAnnotationURL) {
			var oAnnotationMockServer = new MockServer({
				rootUri: "/sap/opu/odata/IWFND/CATALOGSERVICE;v=2/Annotations(TechnicalName='Z_C_PURCHASEDOCUMENTLRP_CDS_VAN',Version='0001')/$value/"
			});
		...
		_initODataServiceMockServer: function (sMetadataURL, sMockDataURL) {
			var oMockServer = new MockServer({
				rootUri: "/sap/opu/odata/sap/Z_C_PURCHASEDOCUMENTLRP_CDS/"
			});
			...
			oMockServer.attachAfter("GET", function (oEvent) {
				...
				var aEntities = oMockServerSrc.getEntitySetData("Z_C_PurchaseDocumentLrp");
				...
				oMockServerSrc.setEntitySetData("Z_C_PurchaseDocumentLrp", aEntities);

			}, "Z_C_PurchaseDocumentLrp");
		...
		init: function () {
			...
			var sAnnotationURL = jQuery.sap.getModulePath(sAppNamespace + ".localService", "/Z_C_PURCHASEDOCUMENTLRP_CDS_VAN.xml");
